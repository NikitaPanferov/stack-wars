...
from core.commands.manager import ActionManager
from core.commands.actions.attack import Attack
...
class GameManager(metaclass=SingletonMeta):
    ...
    def start_new_game(self, armies: InitArmiesDTO):
        ...
        return {
            "alliance": [
                [UnitDTO.from_orm(unit) for unit in units]
                for units in self.alliance.units
            ],
            "horde": [
                [UnitDTO.from_orm(unit) for unit in units] for units in self.horde.units
            ],
        }
        ...
        for row in range(count_rows):
            ...
            for i in range(count_units):
                ...
                if i == 0:
                    ...
                    self.action_manager.execute(attackCommand)
                    action = Action(
